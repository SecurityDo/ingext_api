...
func (c *Client) ImportAppTemplate(repoName string) (err error) {
	...
	//metaPath := fmt.Sprintf("applications/meta", parserType)

	contentResp, err := repoService.GetRepoContent(repoID, "applications/meta")
	if err != nil {
		c.Logger.Error("failed to get repo content", "error", err)
		return fmt.Errorf("failed to get repo content: %w", err)
	}
	...
	err = repoService.ImportAppTemplates(repoID, files)
	if err != nil {
		c.Logger.Error("failed to import repo application templates", "error", err)
		return fmt.Errorf("failed to import repo application templates: %w", err)
	}
	return nil
}

func (c *Client) ImportLakeSchemas(repoName string) (err error) {
	...
	contentResp, err := repoService.GetRepoContent(repoID, "schemas/meta")
	if err != nil {
		c.Logger.Error("failed to get repo content", "error", err)
		return fmt.Errorf("failed to get repo content: %w", err)
	}
	...
	err = repoService.ImportLakeSchemas(repoID, files)
	if err != nil {
		c.Logger.Error("failed to import repo schemas", "error", err)
		return fmt.Errorf("failed to import repo schemas: %w", err)
	}
	return nil
}
