...
== platform_pipe_update_processor
...
* compile one script, without running it
* name and script are alternatives, not a pair: a non-empty name validates the *stored* processor of that name and the script sent with it is ignored, so a broken script paired with a deployed name comes back ok
* a name that is not deployed is not a call error either -- ok is false with "processor <name> not found"
* it is a compile and nothing more: a script whose main() would fail on every event still validates, and so does one with no main() at all. A parse error fails, and so does a statement outside a function ("instruction out of main function").
...
* run one script against one sample document, without deploying it
* type names the runtime that reads source, and is what the console sends along with an empty tenant
** "fpl_processor" (also the fallback for an empty or unrecognized type): source is the JSON document envelope below, and newContent is the mutated envelope
** "fpl_receiver": source is the raw payload, and newContent is {"docs": [...]}
* name is not looked up: the script under test is the one in script, deployed or not
* a script that fails the document is not a call failure -- the message comes back in error with an empty status. An empty source is: the verdict is ERROR, "source is empty".
...
* the source of an fpl_processor is a JSON *string* holding the document the script is handed as main({obj, size}), not a nested object. size is the byte length of obj, and props is {} rather than null.

----
{"obj": {"@cloudtrail": {...}}, "props": {}, "size": 1333, "source": ""}
----
